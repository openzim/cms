...
def test_update_zimfarm_task_status_success(
    ...
    task1 = create_title_upload(status="requested")
    task2 = create_title_upload(status="scraper_running")
    ...
    with patch(
        ...
        update_title_uploads_status(dbsession)
...
def test_update_zimfarm_task_status_fetch_error(
    ...
    task = create_title_upload(status="requested")
