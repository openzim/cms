...
class BaseTitleCollectionSchema(BaseModel):
    collection_name: NotEmptyString = Field(validation_alias="name")
    path: str
...
class TitleUploadLightSchema(BaseModel):
    ...
    status: str
    updated_at: datetime
    s3_key: str | None = Field(exclude=True)
    recipe_id: UUID = Field(exclude=True)
    book_id: UUID | None

    @computed_field
    @property
    def zimfarm_link(self) -> str | None:
        if self.status == "requested":
            return None
        return construct_task_link(self.id)
