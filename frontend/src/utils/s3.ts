...
export class S3MultipartUpload {
  ...
  async uploadPart(partNumber: number, chunk: Blob): Promise<string | undefined> {
    ...
    try {
      const response = await axios.put(presignedEntry.url, chunk, {
        headers: {
          'Content-Type': '',
        },
        signal: this.abortController.signal,
        timeout: 300000, // 5 minutes per 5 MB chunk
      })
