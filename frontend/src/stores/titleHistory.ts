...
export const useTitleHistoryStore = defineStore('titleHistory', () => {
  const history = ref<TitleHistorySchema[]>([])
  ...
  const fetchHistory = async (titleName: string, limit: number, skip: number) => {
    ...
    try {
      ...
      console.error('Failed to fetch title history', _error)
  ...
  const fetchHistoryEntry = async (titleName: string, historyId: string) => {
    ...
    try {
      ...
      console.error('Failed to fetch title history entry', _error)
      errors.value = translateErrors(_error as ErrorResponse)
      return null
    }
  }
  ...
  const revertToHistory = async (titleName: string, historyId: string, comment?: string) => {
    ...
    try {
      ...
      console.error(`Failed to revert to title history entry ${historyId}`, _error)
      errors.value = translateErrors(_error as ErrorResponse)
      return false
    }
  }
