...
export const useCollectionHistoryStore = defineStore('collectionHistory', () => {
  const history = ref<CollectionHistory[]>([])
  ...
  const fetchHistory = async (collectionName: string, limit: number, skip: number) => {
    ...
    try {
      ...
      console.error('Failed to fetch collection history', _error)
  ...
  const fetchHistoryEntry = async (collectionName: string, historyId: string) => {
    ...
    try {
      ...
      console.error('Failed to fetch collection history entry', _error)
      errors.value = translateErrors(_error as ErrorResponse)
      return null
    }
  }
  ...
  const revertToHistory = async (collectionName: string, historyId: string, comment?: string) => {
    ...
    try {
      ...
      console.error(`Failed to revert to collection history entry ${historyId}`, _error)
      errors.value = translateErrors(_error as ErrorResponse)
      return false
    }
  }
