  ...
  <v-dialog v-model="isOpen" max-width="600px" persistent>
    <v-card>
      <v-card-title class="text-h5 pa-4 bg-primary">
        <span class="text-white">Unpromote Book</span>
      </v-card-title>
      ...
      <v-card-text class="pt-4">
        <div>
          ...
          <v-alert type="info" density="compact" class="mb-4" variant="tonal">
            This book will be moved back to Staging.
          </v-alert>

          <v-alert type="warning" density="compact" class="mb-4" icon="mdi-alert" variant="tonal">
            Unpromoting this book from Production to Staging will trigger file operations. This
...
async function handleSubmit() {
  ...
  const book = await bookStore.unPromoteBook(props.book.id)
  if (book) {
    emit('moved')
    isOpen.value = false
  } else {
