  ...
  <v-container fluid>
    <template v-if="loading && history.length === 0">
      <div class="d-flex flex-column justify-center align-center pa-8">
        ...
        <span class="mt-4">Loading Title history...</span>
      </div>
    </template>
    ...
    <template v-else>
      <v-row>
        <v-col cols="12" class="d-flex flex-column align-center justify-center">
          ...
          <div class="text-h6 text-medium-emphasis">No history available for this title</div>
        </v-col>
      </v-row>
    </template>
      ...
      <template #content>
        <div v-if="selectedHistoryItem">
          <v-alert type="info" variant="tonal" density="compact" class="mb-4">
            You are about to revert title <strong>{{ titleName }}</strong> to a previous version.
...
const toggleHistoryItemSelection = async (item: TitleHistorySchema, index: number) => {
  ...
  if (missingIds.length > 0) {
    try {
      ...
      const fetchPromises = missingIds.map((id) =>
        titleHistoryStore.fetchHistoryEntry(props.titleName, id),
      )

      await Promise.all(fetchPromises)

      // After fetching, find the indices again
      const newIndex1 = titleHistoryStore.history.findIndex(
        (item: TitleHistorySchema) => item.id === id1,
      )
      const newIndex2 = titleHistoryStore.history.findIndex(
        (item: TitleHistorySchema) => item.id === id2,
      )
...
const confirmRevert = async (item: TitleHistorySchema, index: number) => {
  ...
  if (!titleDifferences.value || titleDifferences.value.length === 0) {
...
const handleRevertConfirm = async () => {
  ...
  const success = await titleHistoryStore.revertToHistory(
    props.titleName,
  ...
  if (success) {
    ...
    for (const error of titleHistoryStore.errors) {
