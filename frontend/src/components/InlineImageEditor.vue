...
const loadImage = (dataUrl: string) => {
  ...
  img.onerror = () => {
    resetState()
    internalErrorMessage.value = 'Failed to load image. Please try a different image.'
  }
  img.src = dataUrl
}
...
// Watch for changes in image data and emit base64
watch(internalImageData, (newContent) => {
  if (newContent) {
