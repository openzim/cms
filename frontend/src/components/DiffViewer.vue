                ...
                <v-card :class="getDiffLineClass(item.type)" flat class="mb-1">
                  <v-card-text class="pa-1 font-monospace">
                    <div class="d-flex flex-column flex-sm-row mb-1 align-items-sm-center">
                      ...
                      <div class="text-body-2 text-break flex-grow-1 d-flex d-sm-none">
                        <template v-if="item.type === 'modified'">
                          <span class="text-medium-emphasis">Modified blob content</span>
                        </template>
                        <template v-else-if="item.type === 'added'">
                          <span class="text-success">Added blob content</span>
                        </template>
                        <template v-else>
                          <span class="text-error">Removed blob content</span>
                        </template>
                      </div>
                    ...
                    <div v-if="isExpanded(index)" class="mt-2 d-none d-sm-block">
                      <BlobDiffViewer
                        :old-blob-content="item.oldBlobContent"
                        :new-blob-content="item.newBlobContent"
    ...
    switch (diff.kind) {
      case 'N': // New property added
        items.push({
          ...
          path: pathStr,
          oldBlobContent: null,
          newBlobContent: diff.rhs ? String(diff.rhs) : null,
        })
        break
      ...
      case 'D': // Property deleted
        items.push({
          type: 'removed',
          path: pathStr,
          oldBlobContent: diff.lhs ? String(diff.lhs) : null,
          newBlobContent: null,
        })
        break
